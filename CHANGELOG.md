...
no `server_default`) so ORM flushes batch into one INSERT. shared-auth-lib defines no
models; no code here changes. Consumers relocking must apply the autogenerated migration
dropping `DEFAULT gen_random_uuid()` on every `id` column.
